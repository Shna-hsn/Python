...
def get_FileList(folderpath):
    ...
    if pidcount == 1:
        ...
        for filename in fileslisttemp:
            ...
            if filecreatetime > datetime.datetime.now() - datetime.timedelta(minutes=3):
...
#                moveFile(sourcefile,destifile)
            ...
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                ...
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif 'FP_XY_PG' in fileslist[i]:
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                fp_xy_filelist.append(fileslist[i])
                print "FP_XY"
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sep+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
            ...
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                ...
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('FG_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                fg_filelist.append(fileslist[i])
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('UD_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                ud_filelist.append(fileslist[i])
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('LD_PCB_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                fb_filelist.append(fileslist[i])
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('LD_PSA_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                ld_filelist.append(fileslist[i])
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('HS_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                ff_filelist.append(fileslist[i])
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('FA_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                fa_filelist.append(fileslist[i])
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('MB_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                mb_filelist.append(fileslist[i])
                print "MB"
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('TOP_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                top_filelist.append(fileslist[i])
                print "TOP"
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                destifile = errorfolderdt+os.sep + fileslist[i]
                moveFile(sourcefile,destifile)
        elif ('BOT_PG' in fileslist[i]):
            if ('~lock' not in fileslist[i]) or ('._' not in fileslist[i]):
                bot_filelist.append(fileslist[i])
                print "BOT"
            else:
                #shutil.move(folderpath+os.sep+fileslist[i],errorfolderdt+os.sepp+fileslist[i])
                sourcefile = folderpath+os.sep+fileslist[i]
                ...
                moveFile(sourcefile,destifile)
#move error file
#        elif ('|NoSFC|' in fileslist[i]) or ('KeyCap_PG' in fileslist[i]) or ('Character_PG' in fileslist[i]):
        elif ('KeyCap_PG' in fileslist[i]) or ('Character_PG' in fileslist[i]):
            sourcefile = folderpath+os.sep+fileslist[i]
            destifile = errorfolderdt+os.sep + fileslist[i]
            moveFile(sourcefile,destifile)
...
ht_filelist,ht_r_filelist,wf_filelist,wf_r_filelist,uic_filelist,uic_r_filelist,hs_filelist,hs_r_filelist,\
 fp_filelist,fp_xy_filelist,emr_filelist,co_filelist,co_r_filelist,mr_filelist,kf_filelist,muk_filelist,muk_psa_filelist,isakf_filelist,\
test_buck_filelist,shim_filelist,fg_filelist,ud_filelist,fb_filelist,ld_filelist,ff_filelist,fa_filelist,mb_filelist,top_filelist,bot_filelist

def impalaConnection(hostname,port):
    hostname1 = 'p8cdhdatap05.wzs.wistron'
    hostname2 = 'p8cdhdatap06.wzs.wistron'
    try:
...
if __name__ == "__main__":
    try:
        pid_id = 12
        pid_folder = '/home/armap/kuduprogram/p8_AOI_upload_data_buffer_tmp'
        file_folder = '/data/buffer/kudu_aoi_csv_tmp/'+str(pid_id)
#        backupfolder = '/data/history/csv/aoi_csv'
        backupfolder = '/data/buffer/hbase_aoi_csv'
        ...
        ht_filelist,ht_r_filelist,wf_filelist,wf_r_filelist,uic_filelist,uic_r_filelist,hs_filelist,hs_r_filelist,\
        fp_filelist,fp_xy_filelist,emr_filelist,co_filelist,co_r_filelist,mr_filelist,kf_filelist,muk_filelist,muk_psa_filelist,\
        isakf_filelist,test_buck_filelist,shim_filelist,fg_filelist,ud_filelist,fb_filelist,ld_filelist,ff_filelist,fa_filelist,mb_filelist,top_filelist,bot_filelist = get_FileList(file_folder)
        if kb_filelist !=[]:
            func = 'KB'
        ...
        if fg_filelist != []:
            func = 'FG'
            func_filelist = fg_filelist
            handle(func,func_filelist)
        if ud_filelist != []:
            func = 'UD'
            func_filelist = ud_filelist
            handle(func,func_filelist)
        if fb_filelist != []:
            func = 'FB'
            func_filelist = fb_filelist
            handle(func,func_filelist)
        if ld_filelist != []:
            func = 'LD'
            func_filelist = ld_filelist
            handle(func,func_filelist)
        if ff_filelist != []:
            func = 'FF'
            func_filelist = ff_filelist
            handle(func,func_filelist)
        if fa_filelist != []:
            func = 'FA'
            func_filelist = fa_filelist
            handle(func,func_filelist)
        if mb_filelist != []:
            func = 'MB'
            func_filelist = mb_filelist
            handle(func,func_filelist)
        if top_filelist != []:
            func = 'TOP'
            func_filelist = top_filelist
            handle(func,func_filelist)
        if bot_filelist != []:
            func = 'BOT'
            func_filelist = bot_filelist
            handle(func,func_filelist)
