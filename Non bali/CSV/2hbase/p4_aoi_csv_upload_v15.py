...
source_path = '/data/buffer/hbase_aoi_csv/'
...
#        if "|NoSFC|" not in csv_file[i]:
         if "|IGERMA|" not in csv_file[i]: 
           if "|MRCALIPER_PG|" not in csv_file[i]:
            if "~lock" not in csv_file[i]:
             if "|Shim2D_PG|" not in csv_file[i]:
              if "|VoidBlob_PG|" not in csv_file[i]:
               if "|JIS_HT_Main|" not in csv_file[i]:
                if "|VoidBlobResult_PG|"  not in csv_file[i]:
                 if "HT_Repair_" not in csv_file[i]:
                  if "|VoidCoverArea_PG|" not in csv_file[i]:
                   if "LASER_CUT" not in csv_file[i]: 
                    if "|CalDistanceResult_PG|" not in csv_file[i]:
                     if "|IMUK_PG|" not in csv_file[i]:
                      if "|KeyCap_PG_Height|" not in csv_file[i]:
                       if "|KCAttach_PG|" not in csv_file[i]:
                        if "|KeyCap_PG_XY|" not in csv_file[i]:
                         if "|as|" not in csv_file[i]:
...
#    MO    = csv_file[i].split("|")[3].split("_")[0]
                                    ...
                                    if SN > 16:
                                        CDATE = csv_file[i].split("|")[-1].split("_")[-1].split(".")[0]
